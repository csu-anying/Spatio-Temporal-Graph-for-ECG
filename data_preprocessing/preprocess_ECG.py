...
def load_dataset(path, sampling_rate, release=False):
    ...
    elif path.split('/')[-2] == 'CPCS':
        # load and convert annotation data
        Y = pd.read_csv(path + 'cpsc_database2.csv', index_col='ecg_id')
        Y.scp_codes = Y.scp_codes.apply(lambda x: ast.literal_eval(x))
...
def load_raw_data_cpsc(df, sampling_rate, path):
    if sampling_rate == 100:
        if os.path.exists(path + 'raw100.npy'):
            ...
            data = np.array([pre_data_slice(signal) for signal, meta in data])
...
def compute_label_aggregations(df, folder, ctype):
    ...
    if ctype in ['diagnostic_MI', 'diagnostic', 'subdiagnostic', 'superdiagnostic']:
        def find_matching_labels(diagnostic_list):
            # 返回所有匹配的标签
            target_labels = ['NORM', 'LMI', 'AMI', 'ILMI', 'ALMI', 'IPMI', 'IPLMI', 'PMI', 'IMI',
                             'ASMI', 'INJIN', 'INJIL', 'INJAS', 'INJAL', 'INJLA']
            return [label for label in diagnostic_list if label in target_labels]
        ...
        elif ctype == 'diagnostic_MI':
            df['diagnostic'] = df.scp_codes.apply(aggregate_all_diagnostic)
            df['diagnostic_MI'] = df['diagnostic'].apply(find_matching_labels)
            df['diagnostic_MI_len'] = df.diagnostic_MI.apply(lambda x: len(x))
...
def select_data(XX, YY, ctype, min_samples):
    ...
    if ctype == 'diagnostic':
        ...
        X = XX[YY.diagnostic_MI_len > 0]
        Y = YY[YY.diagnostic_MI_len > 0]
        mlb.fit(Y.diagnostic_MI.values)
        y = mlb.transform(Y.diagnostic_MI.values)
    elif ctype == 'subdiagnostic':
...
def pre_data_slice(signal):
    if signal.shape[0] < 1000:
        # dat = np.pad(dat, (0, 1000 - dat.shape[0]), 'constant', constant_values=0)
        signal = resample(signal, 1000, axis=0)
